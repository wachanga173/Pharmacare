...
function loadNextProducts() {
  ...
  const numToSelect = Math.min(
    productsPerView,
    carouselState.availableProducts.length,
  );
  ...
  for (let i = 0; i < numToSelect; i++) {
    ...
    const randomIndex = Math.floor(
      Math.random() * carouselState.availableProducts.length,
    );
...
function renderProducts() {
  ...
  setTimeout(() => {
    track.innerHTML = currentProducts
      .map(
        (product, index) => `
    ...
    showToast(
      `
            <div style="display:flex;align-items:center;gap:1rem;">
                <img src="${product.image || "assets/images/products/placeholder.png"}" alt="${product.name}" style="width:40px;height:40px;object-fit:cover;border-radius:0.375rem;">
