...
export async function getProducts() {
  ...
  // Fallback to localStorage if no products from Supabase
  if (!products || products.length === 0) {
    const storedProducts = getFromStorage(window.CONFIG.STORAGE_KEYS.PRODUCTS);
    if (storedProducts && storedProducts.length > 0) {
      console.log(
        "Using cached products from localStorage:",
  ...
  // Always save products to localStorage for cart.js compatibility
  if (products && products.length > 0) {
